...
// // // Get all items for the admin dashboard

// // export const getAdminItems = async (req, res) => {
// //     try {
// //         console.log("Attempting to fetch admin items...");
        
// //         // Ensure your Item model and its owner path are correct
...
// //         res.json({ success: true, items });
// //     } catch (error) {
// //         // Log the actual server-side error to the console
// //         console.error("Error in getAdminItems:", error.message);
// //         res.json({ success: false, message: "Error fetching items." });
// //     }
// // };

// // Get all items for the admin dashboard WITH PAGINATION AND SEARCH
// export const getAdminItems = async (req, res) => {
//     try {
//         console.log("Attempting to fetch admin items...");
        
//         // ⭐ 1. Get pagination and search parameters from query string ⭐
//         const page = parseInt(req.query.page) || 1;
//         const limit = parseInt(req.query.limit) || 10; // Fixed limit of 10
//         const skip = (page - 1) * limit;

//         const searchTerm = req.query.search || '';

//         // 2. Define the base filter (empty for all items)
//         let filter = {};
        
//         // ⭐ 3. Add search filter if a search term exists ⭐
//         if (searchTerm) {
//             // This is complex because 'owner' is a reference and 'name' is in the User model.
//             // For simplicity, we'll search by brand and model directly.
//             // If you want to search by owner name, you must first find matching user IDs.

//             // To allow searching on brand/model:
//             const searchRegex = new RegExp(searchTerm, 'i'); // Case-insensitive search
//             filter = {
//                 $or: [
//                     { brand: searchRegex },
//                     { model: searchRegex }
//                     // To search by owner name, you'd need a separate aggregation pipeline or an initial User query:
//                     // { 'owner.name': searchRegex } // This requires aggregation/lookup
//                 ]
//             };
//         }

//         // ⭐ 4. Count total items for pagination math (using the filter) ⭐
//         const totalItems = await Item.countDocuments(filter);
//         const totalPages = Math.ceil(totalItems / limit);

//         // ⭐ 5. Fetch the items for the current page using skip() and limit() ⭐
//         const items = await Item.find(filter) 
//             .populate('owner', 'name') // Populate the owner's name for the table
//             .sort({ createdAt: -1 })
//             .skip(skip)
...
//         console.log("Items fetched successfully, Page:", page);

//         // ⭐ 6. Return items along with pagination metadata ⭐
//         res.json({ 
//             success: true, 
//             items,
//             currentPage: page,
